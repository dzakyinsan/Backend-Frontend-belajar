...
function ManageUser() {
    ...
    return datausers.map((val, index) => {
      return (
        <tr key={index}>
          ...
          <td>{val.username}</td>
          <td>
            <img src={`${APIURLimage + val.image}`} alt={index} width="100px" height="100px" />
          </td>
          <td>{val.email}</td>
          <td>{val.phone}</td>
          <td>{val.usia}</td>
          {val.rolename ? <td>{val.rolename}</td> : <td>user tidak ada peran tolong diisi</td>}
          <td>
  ...
  const oneditImageFileChange = event => {
    ...
    console.log("event.target.files[0]", event.target.files[0]); //ini isinya object
